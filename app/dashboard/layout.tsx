...
import { usePathname } from 'next/navigation'; // Monitora qual página está aberta
...
export default function DashboardLayout({
  ...
  // Lista de páginas que APENAS administradores podem acessar
  ...
  // Verifica se o usuário atual é vendedor e está tentando acessar uma rota proibida
    ...
    <div className="flex min-h-screen bg-[#E0DDDD]/20 flex-col md:flex-row">
      ...
      <aside className={`fixed inset-y-0 left-0 w-72 md:w-64 bg-[#6A283A] text-white flex flex-col justify-between p-4 shadow-2xl z-40 transform transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${menuAberto ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="overflow-y-auto">
          ...
          {/* Informações do Perfil Logado no Menu */}
          ...
          {/* Links de Navegação Filtrados por Cargo */}
      ...
      {/* Área de Conteúdo com Trava de Segurança */}
      <main className="flex-1 p-4 md:p-8 overflow-y-auto w-full max-w-full">
        {acessoBloqueado ? (
          /* 🛑 TELA DE ACESSO NEGADO CASO O VENDEDOR TENTE BURLAR A URL */
          ...
          /* Permite visualizar a página normalmente se for Admin ou se for uma rota livre */
          children
        )}
      </main>
    </div>
